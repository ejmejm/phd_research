"""Block-sparse MLP stored as K independent sub-MLPs.

Where ``PaddedMLP`` expresses block-sparsity as a mask over a dense matrix,
this stores the blocks themselves: each sub-MLP owns one slice of the input
and one slice of the output, and the forward pass is a batched einsum over
the task axis. Same function, but the cost scales with the block sizes rather
than with the full dense matrix, which is what makes small block-sparse runs
fast enough to iterate on.

The paper's figures use ``PaddedMLP`` -- matched parameter counts against a
dense network are easier to reason about when both run the same matmul. Use
this when you want block-sparse results quickly and do not need that
comparison.
"""
import math
