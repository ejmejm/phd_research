...
Comet is what the paper's sweeps use; the MLflow and W&B paths are kept
because they cost nothing (both are imported lazily) and let a fork log
wherever it already has infrastructure.

The Comet workspace is resolved by comet_ml itself, from COMET_WORKSPACE or
~/.comet.config, so it never appears in a tracked file.
"""
...
def init_experiment(project: str, config: Optional[DictConfig],
    ...
    if comet_sweep_id or (config and config.get('comet_ml', False)):
        ...
        # Falls back to COMET_WORKSPACE / ~/.comet.config, so the workspace
