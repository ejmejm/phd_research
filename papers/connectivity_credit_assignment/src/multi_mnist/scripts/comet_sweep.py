"""Register a Comet Optimizer sweep, and run agents against it.

    comet_sweep <config.yaml>     # register the grid; prints a sweep id
    comet_sweep --run <sweep-id>  # pull and run trials until the grid is done

An agent is just a process pulling trials, so a cluster array job is N of
these against one sweep id. See experiments/sweeps/README.md.
"""
import ast
import argparse
import copy
