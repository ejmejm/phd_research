...
def train_step(
    ...
    def loss_fn(model):
        ...
        loss_per_batch = _per_task_loss(outputs_r, one_hot, loss_type)  # (B, K)
...
def run_experiment(cfg: DictConfig, train_state: TrainState, streams,
    ...
    # (C) When structure_search is disabled, the structure is fixed for the
    # whole run, so structure diagnostics are constant — compute once and
    # reuse instead of re-syncing the GPU every log period.
    static_structure_metrics = None
    ...
    # (A) Prefetch the next batch on a worker thread while the GPU runs the
    # current scan. Single worker → sequential calls to sample_batch, so the
    # stream's rng/permutation state is touched from one thread at a time.
    def _prepare_batch():
    ...
    for _ in range(num_log_periods):
        ...
        # Kick off the next batch prep on CPU before launching this scan.
        next_batch_future = prefetch_executor.submit(_prepare_batch)
        imgs = jnp.array(imgs_np)
        lbls = jnp.array(lbls_np)
        ...
        if structure_search_enabled:
            ...
            structure_metrics = dict(static_structure_metrics)
