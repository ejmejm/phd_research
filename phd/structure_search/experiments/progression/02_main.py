"""Step 2 — Random Feature Search main run at the best LR from 02_lr_sweep.
...
STUDY_NAME = f'{MLFLOW_PROJECT}/step2_lr_sweep'
RUN_NAME = 'step2_random_feature_main'
...
def best_lr(study: optuna.Study) -> float:
    ...
    if not completed:
        raise RuntimeError(
            f'No completed trials in {STUDY_NAME}. Run 02_lr_sweep.py first.')
