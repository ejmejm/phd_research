...
def main(cfg: DictConfig) -> None:
    ...
    for seed in seeds:
        ...
        run_vars.append(prepare_ltu_geoff_experiment(cfg))
        
    # Need to deep copy first optimizer, then copy the state of the other optimizers into it as a hack.
        
    tasks, models, criteria, optimizers, repr_optimizers, cbp_trackers, rngs = zip(*run_vars)
        
    print('a')
    tasks, models, criteria, optimizers, repr_optimizers, cbp_trackers, rngs = \
        [jax.tree.map(lambda *inner_args: jnp.stack(inner_args), *args) for args in zip(*run_vars)]
