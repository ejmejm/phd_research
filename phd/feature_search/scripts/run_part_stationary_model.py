...
class TrainState(eqx.Module):
    ...
    def __init__(self, cfg, criterion, model, optimizer, repr_optimizer, cbp_tracker, distractor_tracker, rng):
        ...
        self.learning_feature_idx = jnp.array(-1)
        self.learning_layer_idx = jnp.array(-1)
...
def filter_grads_by_feature_and_layer(grads, train_state):
    ...
    if learning_feature_idx == -1 and learning_layer_idx == -1:
        return grads
    
    # Apply layer mask if specified using equinox.filter
    if learning_layer_idx != -1:
        # Create a filter spec that filters out layers we don't want to update
        base_filter_spec = jax.tree.map(lambda _: True, grads)
        
        if learning_layer_idx == 0:
            # Only update layer 0, filter out layer 1
            filter_spec = eqx.tree_at(
                lambda g: g.layers[1],
                base_filter_spec,
                jax.tree.map(lambda _: False, grads.layers[1]),
            )
        elif learning_layer_idx == 1:
            # Only update layer 1, filter out layer 0
            filter_spec = eqx.tree_at(
                lambda g: g.layers[0],
                base_filter_spec,
                jax.tree.map(lambda _: False, grads.layers[0]),
            )
        else:
            filter_spec = base_filter_spec
        
        # Filter gradients: replace False entries with None
        grads = eqx.filter(grads, filter_spec, replace=None)
    
    # Apply feature mask if specified
    if learning_feature_idx != -1:
        # Layer 0 (input layer): shape is (hidden_dim, input_dim)
        # Mask should be True only for row learning_feature_idx (input weights for that feature)
        layer_0_grad = grads.layers[0].weight
        if layer_0_grad is not None and layer_0_grad.ndim == 2:
            # Create a filter spec: True only for the selected row
            mask_0 = jnp.zeros_like(layer_0_grad, dtype=bool)
            mask_0 = mask_0.at[learning_feature_idx, :].set(True)
            # Use equinox.filter with the mask to set unused elements to None
            # Note: equinox.filter works element-wise when filter_spec matches array shape
            filtered_layer_0 = eqx.filter(layer_0_grad, mask_0, replace=None)
            grads = eqx.tree_at(lambda g: g.layers[0].weight, grads, filtered_layer_0)
        
        # Layer 1 (output layer): shape is (output_dim, hidden_dim)
        # Mask should be True only for column learning_feature_idx (output weights for that feature)
        layer_1_grad = grads.layers[1].weight
        if layer_1_grad is not None and layer_1_grad.ndim == 2:
            # Create a filter spec: True only for the selected column
            mask_1 = jnp.zeros_like(layer_1_grad, dtype=bool)
            mask_1 = mask_1.at[:, learning_feature_idx].set(True)
            # Use equinox.filter with the mask to set unused elements to None
            filtered_layer_1 = eqx.filter(layer_1_grad, mask_1, replace=None)
            grads = eqx.tree_at(lambda g: g.layers[1].weight, grads, filtered_layer_1)
...
def train_step(
    ...
    # TODO: Update learning feature and layer indices
    if cfg.train.get('step_per_learning_feature_switch', -1) > 0:
        pass
    
    if cfg.train.get('step_per_learning_layer_switch', -1) > 0:
        pass
